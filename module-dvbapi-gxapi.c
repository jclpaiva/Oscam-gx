...
static void FilterSectionCallback(int SectionHandle, void *UNUSED(AuxParam_p), unsigned char *SectionData_p, unsigned short SectionLen)
{
	...
	{
		if (Filter[i].bOpen &&\
			(Filter[i].hFilter == (unsigned int)SectionHandle))
		{
			pFilter = &Filter[i];
			break;
		}
...
int gxapi_close_filter(unsigned int FilterHandle)
{
	...
	SAFE_MUTEX_UNLOCK(&filter_lock);
	//cs_log("%s OUT;FilterHandle = 0x%X \n",__FUNCTION__,FilterHandle);
	return 0;
...
int gxapi_open_desc(gxapi_desc_open *pOpenParams, unsigned int *pDescHandle)
{
	...
	//cs_log("Demuxid = %d, Pid = %d\n",pOpenParams->DemuxID,pOpenParams->Pid);
	...
	{
		Inst_p = &(DescInst[i]);
		if ((Inst_p->Valid == TRUE) && ((unsigned int)Inst_p->DemuxId == pOpenParams->DemuxID) && (Inst_p->Pid == pOpenParams->Pid))
		{
			*pDescHandle = i;
...
again:
	...
	{
		cs_log("Error, descambler already closed \n");
		SAFE_MUTEX_UNLOCK(&desc_lock);
		return -1;
	}
...
int gxapi_set_desc(int demux_id, unsigned short StreamPid, unsigned char *pOddCW, unsigned char *pEvenCW)
{
	...
	//cs_log("stream pid = %d, demux_id = %d\n",StreamPid, demux_id);
	...
	{
		Inst_p = &(DescInst[i]);
		if ((Inst_p->Valid == TRUE) && (Inst_p->DemuxId == demux_id) && ((Inst_p->Pid == StreamPid) || (8192 == StreamPid)))
		{
